...
clintIo.on('successMessage', (data) => {
    ...
    div.dir = 'rtl';
    const imagePath = globalProfile.profileImage ? `${baseURL}/upload/${globalProfile.profileImage}` : avatar;
    div.innerHTML = `
...
clintIo.on("newMessage", (data) => {
    ...
    if (from?.profileImage) {
        imagePath = `${baseURL}/upload/${from.profileImage}`
    }
    ...
    if (chat.messages?.length) {
        ...
        for (const message of chat.messages) {
            ...
            if (message.createdBy?._id.toString() == globalProfile._id.toString()) {
                ...
                div.className = 'myFriend p-2';
                div.dir = 'ltr';
                const friendImage = message.createdBy.profileImage ? `${baseURL}/upload/${message.createdBy.profileImage}` : avatar
                div.innerHTML = `
                <img class="chatImage" src="${friendImage}" alt="" srcset="">
                <span class="mx-2">${message.content}</span>
                `;
...
function displayGroupChat(groupId) {
    ...
    axios({
        ...
        console.log({ chat });
        if (chat) {
            meImage = globalProfile.profileImage ? `${baseURL}/upload/${globalProfile.profileImage}` : avatar
...
function getUserData() {
    axios({
        ...
        if (user.profileImage) {
            imagePath = `${baseURL}/upload/${user.profileImage}`
        }
...
function showUsersData(users = []) {
    ...
    for (let i = 0; i < users.length; i++) {
        ...
        if (users[i].profileImage) {
            imagePath = `${baseURL}/upload/${users[i].profileImage}`
        }
        console.log({frinds:users[i]});
